...
import { CalendarClock, Plus, Power, Trash2, X, Package, Coffee, Image as ImageIcon, UploadCloud, Pencil } from 'lucide-react';
...
export default function PreorderManagement() {
  ...
  const uploadItemImage = async (index: number, file?: File) => {
    ...
    try {
      ...
      const response = await authFetch('/api/preorders/upload-image', { method: 'POST', body });
      const data = await response.json();
      if (!response.ok) throw new Error(data.message || 'Upload foto gagal');
      updateItem(index, 'image_url', data.url);
    } catch (cause: any) {
      setError(cause.message || 'Upload foto gagal');
    } finally {
  ...
  const startEdit = (campaign: Campaign) => {
    const localDate = (value: string) => { const date = new Date(value); return Number.isFinite(date.getTime()) ? new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16) : ''; };
    setEditingCampaign(campaign);
    setOutlet(campaign.outlet);
    setForm({ name: campaign.name, description: campaign.description || '', outlet: campaign.outlet, order_start_at: localDate(campaign.order_start_at), order_deadline_at: localDate(campaign.order_deadline_at), service_at: localDate(campaign.service_at), items: campaign.items.map(item => ({ name: item.name, category: item.category, price: String(item.price), quota_total: String(item.quota_total), image_url: item.image_url || '', description: '' })) });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const response = await authFetch(editingCampaign ? `/api/preorders/${editingCampaign.id}` : '/api/preorders', {
      ...
      setShowForm(false); setEditingCampaign(null); setForm(emptyForm(outlet)); await fetchCampaigns();
            ...
            return (
              <article key={campaign.id} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div className="px-5 pb-5 space-y-2">
                  {campaign.items.map(item => <div key={item.id} className="flex items-center justify-between gap-3 p-3 border border-slate-100 rounded-xl"><div className="flex items-center gap-3">{item.image_url ? <img src={item.image_url} alt={item.name} className="w-12 h-12 rounded-lg object-cover border border-slate-200"/> : <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400"><ImageIcon size={17}/></div>}<div><p className="text-sm font-semibold">{item.name}</p><p className="text-xs text-slate-400">Rp {Number(item.price).toLocaleString()} · sisa {item.remaining_quota}</p></div></div><span className="text-xs font-bold text-primary">{item.quota_sold}/{item.quota_total}</span></div>)}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={submit} className="bg-white w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl">
            <div className="sticky top-0 bg-white z-10 px-6 py-4 border-b flex items-center justify-between"><div><h3 className="font-bold text-lg">{editingCampaign ? 'Edit Program PO' : 'Program PO'} {outlet === 'ngolab' ? 'Ngolab' : 'Coworking'}</h3><p className="text-xs text-slate-400">Outlet tidak dapat dicampur dalam satu program.</p></div><button type="button" onClick={() => setShowForm(false)} className="p-2 text-slate-400"><X size={20} /></button></div>
            <div className="p-6 space-y-5">
              ...
              <div className="border-t pt-5"><div className="flex justify-between mb-3"><h4 className="font-bold text-sm">Menu dalam Program PO</h4><button type="button" onClick={() => setForm({ ...form, items: [...form.items, emptyItem()] })} className="text-xs font-bold text-primary flex gap-1"><Plus size={14}/> Tambah Menu</button></div><div className="space-y-3">{form.items.map((item, index) => <div key={index} className="grid sm:grid-cols-[1fr_130px_110px_auto_36px] gap-3 items-center bg-slate-50 p-3 rounded-xl"><div className="space-y-2"><input required value={item.name} onChange={e => updateItem(index, 'name', e.target.value)} className="h-10 w-full border rounded-lg px-3 text-sm" placeholder="Nama menu"/>{item.image_url && <img src={item.image_url} alt={item.name || 'Preview menu'} className="h-16 w-24 rounded-lg object-cover border border-slate-200"/>}</div><input required type="number" min="1" value={item.price} onChange={e => updateItem(index, 'price', e.target.value)} className="h-10 border rounded-lg px-3 text-sm" placeholder="Harga"/><input required type="number" min="1" value={item.quota_total} onChange={e => updateItem(index, 'quota_total', e.target.value)} className="h-10 border rounded-lg px-3 text-sm" placeholder="Kuota"/><label className="h-10 px-3 rounded-lg bg-white border border-slate-200 text-xs font-bold text-slate-600 flex items-center justify-center gap-1 cursor-pointer"><UploadCloud size={14}/>{uploadingIndex === index ? 'Upload...' : 'Foto'}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" className="hidden" disabled={uploadingIndex !== null} onChange={e => uploadItemImage(index, e.target.files?.[0])}/></label><button type="button" disabled={form.items.length === 1} onClick={() => setForm({ ...form, items: form.items.filter((_, i) => i !== index) })} className="text-rose-500 disabled:opacity-30"><Trash2 size={16}/></button></div>)}</div></div>
            </div>
            <div className="sticky bottom-0 bg-white px-6 py-4 border-t flex justify-end gap-3"><button type="button" onClick={() => setShowForm(false)} className="px-5 h-10 border rounded-lg text-sm">Batal</button><button disabled={saving} className="px-5 h-10 bg-primary text-primary-foreground rounded-lg text-sm font-semibold disabled:opacity-50">{saving ? 'Menyimpan...' : editingCampaign ? 'Simpan Perubahan' : 'Simpan Program PO'}</button></div>
          </form>
        </div>
      )}
    </div>
